...
{block name="content"}
<table style="width:860px;table-layout:fixed;">
	<tr>
		<th colspan="11">{$LNG.mg_overview}<span id="loading" style="display:none;"> ({$LNG.loading})</span></th>
	</tr>
		{foreach $CategoryList as $CategoryID => $CategoryRow}
		{if ($CategoryRow@iteration % 11) === 1}<tr>{/if}
		{if $CategoryRow@last && ($CategoryRow@iteration % 11) !== 0}<td>&nbsp;</td>{/if}
		...
		{if $CategoryRow@last || ($CategoryRow@iteration % 11) === 0}</tr>{/if}
		{/foreach}
</table>
